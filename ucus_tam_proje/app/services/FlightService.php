...
class FlightService {
    ...
    public function all(): array {
        $sql = "SELECT f.id,
                       ...
                       (SELECT json_agg(json_build_object('id', pe.id, 'first_name', pe.first_name, 'last_name', pe.last_name))
    ...
    public function create(array $data): void {
        ...
        try {
            $stmt = $this->pdo->prepare("INSERT INTO flights (departure_airport_id, arrival_airport_id, plane_id, departure_ts, arrival_ts)
            ...
            if (!empty($data['passenger_ids'])) {
                $ins = $this->pdo->prepare("INSERT INTO flight_person (flight_id, person_id) VALUES (:f, :p)");
